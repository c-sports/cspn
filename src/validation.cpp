...
#if defined(NDEBUG)
# error "CSPN Core cannot be compiled without assertions."
#endif
...
public:
    ...
    bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fProofOfStake);
...
const std::string strMessageMagic = "CSPN Signed Message:\n";
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in CSPN
...

    {
        ...
        if ((nSigOpsCost > MAX_STANDARD_TX_SIGOPS_COST) || (nBytesPerSigOp && nSigOpsCost > nSize / nBytesPerSigOp))
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if(nHeight > 20000)  ret = blockValue * 0.80; // 80% of block reward
    return ret;
}
...
static int64_t nTimeCSPNSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // CSPN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_6_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(CSPN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(CSPN): spork is off, skipping transaction locking checks\n");
    }
    ...
    // CSPN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CSPN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (block.IsProofOfStake() && !IsBlockPayeeValid(*block.vtx[1], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(CSPN): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(CSPN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeCSPNSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - CSPN specific: %.2fms [%.2fs (%.2fms/blk)]\n", MICRO * (nTime5 - nTime4), nTimeCSPNSpecific * MICRO, nTimeCSPNSpecific * MILLI / nBlocksTotal);
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (block.nNonce != 0 && fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits, consensusParams))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    if (!CheckBlockHeader(block, state, consensusParams, fCheckPOW && block.nNonce != 0))
        return false;
    ...
    if (block.GetBlockTime() > GetAdjustedTime() + MAX_FUTURE_BLOCK_TIME)
        return state.DoS(100, false, REJECT_INVALID, "bad-cb-time", "coinbase timestamp is too early");
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid)
{
    if (first_invalid != nullptr) first_invalid->SetNull();
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            if (!g_chainstate.AcceptBlockHeader(header, state, chainparams, &pindex, header.nFlags & CBlockIndex::BLOCK_PROOF_OF_STAKE)) {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    if (fCheckPoS && block.nNonce == 0 && !PoSContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(CSPN): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
...
bool CChainState::LoadGenesisBlock(const CChainParams& chainparams)
{
    ...
    try {
        ...
        if (chainparams.NetworkIDString() == CBaseChainParams::DEVNET) {
            ...
            assert(fCheckBlock);
            if (!AcceptBlock(shared_pblock, state, chainparams, nullptr, true, nullptr, nullptr, false))
                return false;
        }
    } catch (const std::runtime_error &e) {
