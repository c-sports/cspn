...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth, AvailableCoinsType nCoinType, bool fUseInstantSend) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (nCoinType == ONLY_COLLATERALS && pcoin->tx->vout[i].nValue != 1337 * COIN)
                continue;
 ...
 */
bool CWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, std::string fromAccount, CValidationState& state)
{
    {
        auto locked_chain = chain().lock();
        LOCK(cs_wallet);
                              ...
                              CAmount nFees)
{
    ...
    for (const COutput& out : setStakeCoins) {
        ...
        BlockMap::iterator it = ::BlockIndex().find(out.tx->hashBlock);
...
bool CWallet::SetStakeCombineThreshold(const int value)
{
    LOCK(cs_wallet);
    WalletBatch batch(*database);
    return batch.WriteStakeCombineThreshold(value);
}
