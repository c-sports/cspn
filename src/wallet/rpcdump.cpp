...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The cspn address for the private key\n"
            "\nResult:\n"
...
static std::string WriteHDKeypath(const std::vector<uint32_t>& path)
{
    std::string ret;
    for (auto i : path) {
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by CSPN Core %s\n", CLIENT_BUILD);
    ...
    obj.push_back(Pair("cspncoreversion", CLIENT_BUILD));
