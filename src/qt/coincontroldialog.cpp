...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    {
        // Bytes
        nBytes = nBytesInputs + ((CoinControlDialog::payAmounts.size() > 0 ? CoinControlDialog::payAmounts.size() + std::max(1, CoinControlDialog::nSplitUTXODummy) : 2) * 34) + 10; // always assume +1 output for change here
        if (fWitness)
        {
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show cspn address. In tree mode, address is not shown again for direct wallet address outputs
