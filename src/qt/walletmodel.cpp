...
#endif
        {   // User-entered cspn address / amount:
...
#endif
            if (!rcp.message.isEmpty()) // Message from normal cspn:URI (cspn:123...?message=example)
                vOrderForm.emplace_back("Message", rcp.message.toStdString());
        }

        auto& newTx = transaction.getWtx();
        std::string rejectReason;
        if (!wallet().commitTransaction(newTx, {} /* mapValue */, std::move(vOrderForm), {} /* fromAccount */, rejectReason))
