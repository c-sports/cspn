...
const int CSPN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CSPN_IPC_PREFIX("cspn:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/cspn-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/cspn-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/cspn-paymentrequest";
#endif
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the cspn: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(CSPN_IPC_PREFIX, Qt::CaseInsensitive)) // cspn: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CSPN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(CSPN_IPC_CONNECT_TIMEOUT);
...
#endif
{
    ...
    // on Mac: sent when you click cspn: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start cspn: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling cspn: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("cspn://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'cspn://' is not a valid URI. Use 'cspn:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(CSPN_IPC_PREFIX, Qt::CaseInsensitive)) // cspn: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid CSPN address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in cspn: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom cspn addresses are not supported
        ...
        // CSPN amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
