...
const char * const BITCOIN_CONF_FILENAME = "cspn.conf";
const char * const BITCOIN_PID_FILENAME = "cspnd.pid";
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty cspn.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure CSPN Core copyright is not removed by accident
    ...
    // Check for untranslated substitution to make sure CSPN Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("CSPN Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The CSPN Core developers";
    }
