...
# cspn_hash
RUN git clone https://github.com/cspnpay/cspn_hash
RUN cd cspn_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cspn
RUN useradd -u ${USER_ID} -g cspn -s /bin/bash -m -d /cspn cspn
...
RUN mkdir /cspn-src && \
  ...
  chown $USER_ID:$GROUP_ID /cspn-src && \
