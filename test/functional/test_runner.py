...
`test/functional/test_framework/test_framework.py:CSPNTestFramework.main`.
...
def main():
    ...
    enable_cspnd = config["components"].getboolean("ENABLE_BITCOIND")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci):
    ...
    try:
        if subprocess.check_output(["pidof", "cspnd"]) is not None:
