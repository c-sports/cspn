...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 CSPN from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 CSPN in block rewards plus fees, but
        ...
        self.sync_all(self.nodes[0:3])
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), Decimal('900') - totalfee, fee_per_byte, self.get_vsize(self.nodes[2].getrawtransaction(txid)))
        assert_equal(self.nodes[0].getbalance(), Decimal('100'))

        # Send 100 CSPN with subtract fee from amount
        txid = self.nodes[2].sendtoaddress(address, 100, "", "", True)
        self.nodes[2].generate(1)
        self.sync_all(self.nodes[0:3])
        node_2_bal -= Decimal('100')
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        node_0_bal = self.check_fee_amount(self.nodes[0].getbalance(), Decimal('200'), fee_per_byte, self.get_vsize(self.nodes[2].getrawtransaction(txid)))
        ...
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), node_2_bal - Decimal('100'), fee_per_byte, self.get_vsize(self.nodes[2].getrawtransaction(txid)))
        assert_equal(self.nodes[0].getbalance(), node_0_bal)

        # Sendmany 100 CSPN with subtract fee from amount
        txid = self.nodes[2].sendmany('from1', {address: 100}, 0, False, "", [address])
        self.nodes[2].generate(1)
        self.sync_all(self.nodes[0:3])
        node_2_bal -= Decimal('100')
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        node_0_bal = self.check_fee_amount(self.nodes[0].getbalance(), node_0_bal + Decimal('100'), fee_per_byte, self.get_vsize(self.nodes[2].getrawtransaction(txid)))
